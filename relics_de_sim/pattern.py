...
class PatternSimulator:
    ...
    def per_electron_light(self, xy: np.ndarray) -> np.ndarray:
        ...
        gamma = self.rng.normal(1.0, self.elec.se_gamma_sigma, size=len(xy))
        light = light * self.elec.single_electron_gain_per_channel * gamma[:, None]
    ...
    def area_pattern(
        ...
        area_low = sim_low_pe * self.elec.pe_gain
        area_high = sim_up_pe * self.elec.pe_gain

        out_event_id = 0
        for j in range(n_events):
            channel_indices = np.repeat(np.arange(C.N_TOTAL_PMT), pe_event[j])
            pe_area = self.rng.normal(
                self.elec.pe_gain, self.elec.pe_gain_std, size=len(channel_indices)
            )
    ...
    def simulate_event_group(
        ...
        return PatternResult(
            ...
            pe_by_area=area_pattern / self.elec.pe_gain,
            pe_info=pe_info,
            keep_mask=keep_mask,
        )
    ...
    def recons_light_pattern(
        ...
        scale = total_pe_by_area[:, None] / (
            self.elec.single_electron_gain_per_channel * self.elec.average_efficiency
        )
        return scale * recon_light * self.elec.single_electron_gain_per_channel
