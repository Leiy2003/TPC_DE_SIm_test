...
for i in range(len(cevns_sim.cevns_points)):
    ...
    cevns_info = np.zeros(
        ...
        dtype=np.dtype([("area", "<f8"), ("st_cor", "<f8"), ("pattern", "<f8"),("e_num", "uint32"),("z", "<f8"),]),
    )
    ...
    cevns_info["z"] = np.linspace(0, 24, len(cevns_points[st_mask]))
    cevns_info["pattern"] = np.sum(
        calculate_poisson_probabilities_log(
    ...
    z_list = cevns_info["z"][score_cevns > b_cevns]
    ...
    if len(cevns_pass_pattern) >=1:
        ...
        cevns_pass_pattern_info['z'] = z_list
        cevns_pass_pattern_info_list.append(cevns_pass_pattern_info)
    
    cevns_fail_pattern_info = cevns_info[score_cevns <= b_cevns]
    cevns_fail_pattern_info_list.append(cevns_fail_pattern_info)

        
cevns_pass_pattern_info_array = np.concatenate(cevns_pass_pattern_info_list)
cevns_fail_pattern_info_array = np.concatenate(cevns_fail_pattern_info_list)
np.save('acceptance_analysis_waveform/wf_pass_pattern_cevns_{}.npy'.format(index), cevns_pass_pattern_info_array)
np.save('acceptance_analysis_waveform/wf_fail_pattern_cevns_{}.npy'.format(index), cevns_fail_pattern_info_array)
